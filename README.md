...
## Цель работы
Межсервисное взаимодействие, асинхронность, внедрение Real-time web

## Порядок показа
Вызвать через insomnia GET списка заявок, затем http-метод асинхронного сервиса (в insomnia и отдельно кнопкой в интерфейсе), показать что в GET списка заявок появился результат с задержкой, потом вызвать новый метод основного сервиса с ключом, чтобы изменить результат. Сделать скриншоты всех изменений в РПЗ по 2025 году.

## Контрольные вопросы
grpc, асинхронность, веб-сервис
